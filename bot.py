...
def get_active_count(doctor_id):
    return len(r.smembers(f"doctor:{doctor_id}:active_clients"))
...
async def handle_receipt(message: types.Message, state: FSMContext):
    ...
    if can_take_new_client(doctor_id):
        ...
        await bot.send_photo(doctor_id, message.photo[-1].file_id,
            caption=f"🧾 НОВЫЙ ЧЕК\n👤 Клиент: {anonymous_id}\n📂 Тема: {TOPICS[topic_key]}\n\n✅ Активных консультаций: {get_active_count(doctor_id)}/{MAX_ACTIVE_PER_DOCTOR}",
            reply_markup=get_doctor_keyboard(user_id))
        await bot.send_message(doctor_id, f"💬 Клиент {anonymous_id} подключён.")
        await message.answer(f"✅ Оплата подтверждена! Врач свяжется с вами.\nВаш ID: {anonymous_id}")
        await state.clear()
    else:
        position = add_to_queue(topic_key, user_id, anonymous_id)
        await bot.send_photo(doctor_id, message.photo[-1].file_id,
            caption=f"🧾 НОВЫЙ ЧЕК\n👤 Клиент: {anonymous_id}\n📂 Тема: {TOPICS[topic_key]}\n\n⏳ ВРАЧ ЗАНЯТ\nКлиент добавлен в очередь. Позиция: {position + 1}")
        await message.answer(f"⏳ Врач занят. Вы в очереди на позиции {position + 1}.\nВаш ID: {anonymous_id}")
        r.set(f"user:{user_id}:payment_status", "queued")
        await state.clear()

# ============================================
# CALLBACK-ЗАПРОСЫ (aiogram 3.x синтаксис)
# ============================================
...
async def end_consultation(call: types.CallbackQuery):
    ...
    await bot.send_message(user_id, "🏁 Консультация завершена. Спасибо!")
    topic_key = r.get(f"user:{user_id}:topic")
    if topic_key:
        ...
        if next_client_id:
            ...
            await bot.send_message(next_client_id, f"✅ Врач освободился! Консультация начинается.\nВаш ID: {next_anonymous_id}")
            await bot.send_message(doctor_id, f"💬 Новый клиент {next_anonymous_id} из очереди подключён.", reply_markup=get_doctor_keyboard(next_client_id))
...
async def do_transfer(call: types.CallbackQuery):
    ...
    if r.get(f"doctor:{doctor_id}:current_client") == str(user_id):
        r.delete(f"doctor:{doctor_id}:current_client")
    await bot.send_message(user_id, f"🔄 Вас перенаправили к {TOPICS[new_topic]}.")
    await bot.send_message(new_doctor_id, f"🆕 Клиент {anonymous_id} перенаправлен к вам.", reply_markup=get_doctor_keyboard(user_id))
    await call.message.answer(f"✅ Клиент {anonymous_id} перенаправлен {TOPICS[new_topic]}")
    await call.answer()

@dp.callback_query(lambda c: c.data.startswith("busy:"))
async def busy_doctor(call: types.CallbackQuery):
    user_id = int(call.data.split(":")[1])
    anonymous_id = r.get(f"user:{user_id}:anonymous_id") or "клиент"
    await bot.send_message(user_id, "⏳ Врач сейчас занят, ответит в ближайшее время.")
    await call.answer(f"Уведомление отправлено {anonymous_id}")

@dp.callback_query(lambda c: c.data == "show_clients")
...
async def switch_client(call: types.CallbackQuery):
    ...
    await call.message.answer(f"✅ Вы переключились на клиента {anonymous_id}.")
    await call.answer()
...
async def chat_messages(message: types.Message):
    ...
    if r.get(f"user:{user_id}:active"):
        ...
        anonymous_id = r.get(f"user:{user_id}:anonymous_id") or "клиент"
        if doctor_id:
            await bot.send_message(int(doctor_id), f"👤 {anonymous_id}: {message.text}", reply_markup=get_doctor_keyboard(user_id))
    elif r.get(f"doctor:{user_id}:current_client"):
        client_id = r.get(f"doctor:{user_id}:current_client")
        if client_id:
            await bot.send_message(int(client_id), f"👨‍⚕️ Врач: {message.text}")
